...
import (
	...
	"github.com/fushengyk/arbix/internal/collector"
...
func main() {
	...
	sugar.Info("🚀 Starting Collector Service...")
	...
	natsutil.EnsureStream(js, domain.StreamMetadata, domain.StreamMetadataSubjects, sugar)

	// Create and start service
	svc, err := collector.NewService(cfg, js, sugar)
	if err != nil {
		sugar.Fatalf("❌ Failed to create collector service: %v", err)
	}

	if err := svc.Start(); err != nil {
		sugar.Fatalf("❌ Failed to start collector: %v", err)
	}
